...
const suite = suiteSchema.parse({ schemaVersion: 1, id: 'test', version: '1', dataset: { id: 'test', version: '1' }, samples: 2, concurrency: 2, timeoutMs: 1000,
  graders: [{ id: 'exact', kind: 'exact', threshold: 1 }],
  gate: { minScore: 1, minPassRate: 1, maxScoreDrop: 0, maxCaseDrop: 0, maxP95LatencyMs: 10000, maxCandidateCostUsd: 1, maxJudgeCostUsd: 1, requireBaseline: true } })
...
function manifest(overrides: Partial<Manifest> = {}): Manifest {
  return { schemaVersion: 1, engine: 'black-box-eval/1', suite, dataset, target: target.identity, judge: null, seed: 42, provenance: { revision: 'test' }, baseline: null, ...overrides }
}
...
test('timeouts, malformed target output, missing usage and judge errors fail closed', async () => {
  ...
  try {
    ...
    const invalid = await runJob(store, malformed, { ...target, execute: async () => ({ output: '4', usage: { ...zeroUsage(), costUsd: -1 } }) })
...
test('real HTTP Candidate and Autoevals Judge clients isolate credentials, usage, costs and spans', async () => {
  const requests: { path: string; auth: string | undefined; model: string; messages: unknown }[] = []
  ...
  const candidate: ModelConfig = { baseURL: `${base}/candidate`, apiKey: 'candidate-secret', model: 'production-candidate', inputUsdPerMillion: 1, outputUsdPerMillion: 2, maxTokens: 100, timeoutMs: 1000 }
  const judgeConfig = { ...candidate, baseURL: `${base}/judge`, apiKey: 'judge-secret', model: 'independent-judge', inputUsdPerMillion: 3 }
  ...
  try {
    ...
    assert.deepEqual(report.usage, { candidate: { inputTokens: 20, outputTokens: 4, costUsd: 0.000028 }, judge: { inputTokens: 40, outputTokens: 10, costUsd: 0.00014 } })
    ...
    const env = Object.fromEntries([['CANDIDATE', candidate], ['JUDGE', judgeConfig]].flatMap(([role, config]) => {
      ...
      return Object.entries({ BASE_URL: c.baseURL, MODEL: c.model, API_KEY: c.apiKey, INPUT_USD_PER_MILLION: c.inputUsdPerMillion,
        OUTPUT_USD_PER_MILLION: c.outputUsdPerMillion, MAX_TOKENS: c.maxTokens, TIMEOUT_MS: c.timeoutMs }).map(([key, value]) => [`EVAL_${role}_${key}`, String(value)])
    }))
