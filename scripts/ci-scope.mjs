...
export function computeScope(changed = {}, manual = '') {
  ...
  if (manual) {
    ...
    blackBoxEval = enabled('blackBoxEval')
  ...
  return {
    ...
    control,
    server,
    black_box_eval: blackBoxEval,
    integration,
    deploy_contract: deployment || openNotebook || release,
    control_deploy: admin || control,
    control_migrations: controlMigrations,
    release,
    checks: [web, admin, control, server, blackBoxEval, deployment, openNotebook, release].some(Boolean),
...
if (process.argv[1]?.endsWith('ci-scope.mjs')) {
  ...
  const result = computeScope({
    ...
    blackBoxEval: enabled('BLACK_BOX_EVAL'),
