...
async def text_search(
    ...
    try:
        scoped = source_ids is not None or note_ids is not None
        if scoped:
            search_results = await repo_query(
                """
                SELECT * FROM fn::scoped_text_search(
                    $keyword, $results, $source, $note, $source_ids, $note_ids
                )
                """,
                {
                    "keyword": keyword,
                    ...
                    "source": source,
                    "note": note,
                    "source_ids": [ensure_record_id(value) for value in (source_ids or [])],
                    "note_ids": [ensure_record_id(value) for value in (note_ids or [])],
                },
            )
        else:
            search_results = await repo_query(
...
async def vector_search(
    ...
    try:
        ...
        scoped = source_ids is not None or note_ids is not None
        function_name = "fn::scoped_vector_search" if scoped else "fn::vector_search"
        parameters = {
            "embed": embed,
            "results": results,
            "source": source,
        ...
        if scoped:
            parameters["source_ids"] = [
                ensure_record_id(value) for value in (source_ids or [])
            ]
            parameters["note_ids"] = [
                ensure_record_id(value) for value in (note_ids or [])
            ]
            query = f"""
            ...
            """
        search_results = await repo_query(query, parameters)
        return search_results
    except Exception as e:
