...
async def resolve_notebook_scope(
    ...
    """Resolve caller input against notebook relationships before any search."""
    notebook = await Notebook.get(notebook_id)
    notebook_sources = {
        source.id for source in await notebook.get_sources() if source.id
    }
    requested = set(requested_source_ids or [])
    allowed_sources = (
        notebook_sources.intersection(requested)
        if requested_source_ids is not None
        else notebook_sources
    )
    allowed_sources.difference_update(excluded_source_ids)
    note_ids = [note.id for note in await notebook.get_notes() if note.id]
    return sorted(allowed_sources), note_ids
...
async def search_knowledge_base(search_request: SearchRequest):
    ...
    try:
        source_ids, note_ids = await resolve_notebook_scope(
            search_request.notebook_id,
            search_request.source_ids,
            search_request.excluded_source_ids,
        )
...
async def stream_ask_response(
    ...
    try:
        ...
        async for chunk in ask_graph.astream(  # type: ignore[call-overload]
            input=dict(question=question, source_ids=source_ids, note_ids=note_ids),
            config=dict(
                configurable=dict(
...
async def ask_knowledge_base(ask_request: AskRequest):
    ...
    try:
        source_ids, note_ids = await resolve_notebook_scope(
            ...
            ask_request.source_ids,
            ask_request.excluded_source_ids,
        )
