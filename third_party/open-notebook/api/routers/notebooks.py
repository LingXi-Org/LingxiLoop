...
def _notebook_response(row: dict) -> NotebookResponse:
    """Keep the internal external key visible to server-side gateways."""
...
async def get_notebooks(
    ...
    try:
        ...
        return [_notebook_response(nb) for nb in result]
...
async def create_notebook(notebook: NotebookCreate):
    """Create a notebook, idempotently when an external key is supplied."""
    try:
        if notebook.external_key:
            existing = await repo_query(
                """
                SELECT *, count(<-reference.in) AS source_count,
                          count(<-artifact.in) AS note_count
                FROM notebook WHERE external_key = $external_key LIMIT 1
                """,
                {"external_key": notebook.external_key},
            )
            if existing:
                return _notebook_response(existing[0])
        ...
        return NotebookResponse(
            ...
            external_key=new_notebook.external_key,
        )
