...
async def _assert_file_supported(file_path: str) -> None:
    ...
    try:
        ...
        if store.is_object_reference(file_path):
            ...
            check_path = str(materialized.path)
        support = await check_file_support(check_path)
...
async def save_uploaded_file(upload_file: UploadFile) -> str:
    """Persist an upload to R2 when configured, otherwise keep it locally."""
    if not upload_file.filename:
        raise ValueError("No filename provided")

    content = await upload_file.read()
    local_path = await asyncio.to_thread(
        _write_uploaded_file, upload_file.filename, content
    )
    try:
        reference = await asyncio.to_thread(
            get_artifact_store().persist_file,
            local_path,
            "sources",
            upload_file.filename,
        )
        if get_artifact_store().is_object_reference(reference):
...
async def _build_content_state(
    ...
    if source_data.type == "link":
        ...
        store = get_artifact_store()
        if store.is_object_reference(final_file_path):
            try:
                ...
                raise HTTPException(status_code=400, detail="Invalid uploaded file reference")
        else:
...
async def create_source(
    ...
    try:
        ...
        await _cleanup_uploaded_file(file_path, upload_file)
        raise
...
async def _resolve_source_file(source_id: str) -> tuple[str, str]:
    ...
    if store.is_object_reference(file_path):
        try:
            if not await asyncio.to_thread(
                store.exists, file_path, "sources", UPLOADS_FOLDER
            ):
                raise HTTPException(status_code=404, detail="File not found on server")
            return file_path, Path(file_path).name
        except ValueError:
            logger.warning(f"Blocked invalid R2 reference for source {source_id}")
...
def _is_source_file_available(source: Source) -> Optional[bool]:
    ...
    if store.is_object_reference(file_path):
        try:
            return store.exists(file_path, "sources", UPLOADS_FOLDER)
        except ValueError:
            return False
    safe_root = os.path.realpath(UPLOADS_FOLDER)
    resolved_path = os.path.realpath(file_path)
...
async def get_source(source_id: str):
    ...
    try:
        ...
        return _source_to_response(
            source,
            embedded_chunks=embedded_chunks,
            file_available=await asyncio.to_thread(_is_source_file_available, source),
            # Status fields
...
async def download_source_file(source_id: str):
    ...
    try:
        ...
        store = get_artifact_store()
        if store.is_object_reference(resolved_path):
            download = await asyncio.to_thread(
                store.open_download, resolved_path, "sources"
            )
            headers = {}
            if download.content_length is not None:
                headers["Content-Length"] = str(download.content_length)
            headers["Content-Disposition"] = (
                f"attachment; filename*=UTF-8''{quote(download.filename)}"
            )
