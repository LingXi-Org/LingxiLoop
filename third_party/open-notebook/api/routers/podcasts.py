...
async def _delete_episode_audio(episode: PodcastEpisode, episode_id: str) -> None:
    """Best-effort delete of local or R2 audio, refusing invalid references.
    ...
    if not episode.audio_file:
        return
    try:
        await asyncio.to_thread(delete_audio_file, episode.audio_file)
        logger.info(f"Deleted audio file for episode {episode_id}")
    except Exception as e:
        logger.warning(
            f"Refusing or failing to delete audio for episode {episode_id}: {e}"
        )
...
async def list_podcast_episodes():
    ...
    try:
        ...
        for episode in episodes:
            ...
            audio_url = None
            if await asyncio.to_thread(audio_file_available, episode.audio_file):
...
async def stream_podcast_episode_audio(episode_id: str):
    ...
    if store.is_object_reference(episode.audio_file):
        try:
            download = await asyncio.to_thread(open_audio_download, episode.audio_file)
        except ValueError:
            raise HTTPException(status_code=403, detail="Access to file denied")
        except Exception as e:
            logger.warning(f"Failed to open R2 audio for episode {episode_id}: {e}")
...
async def retry_podcast_episode(episode_id: str):
    ...
    try:
        ...
        await _delete_episode_audio(episode, episode_id)
