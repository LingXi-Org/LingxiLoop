...
@router.put("/insights/{insight_id}", response_model=SourceInsightResponse)
async def update_insight(insight_id: str, request: SourceInsightUpdate):
    """Update insight content or its human-readable type."""
    try:
        insight = await SourceInsight.get(insight_id)
        if request.insight_type is not None:
            insight.insight_type = request.insight_type
        ...
        raise HTTPException(status_code=500, detail="Error updating insight")
