...
class NotebookCreate(BaseModel):
    ...
    external_key: Optional[str] = Field(
        ...
        description="Stable idempotency key supplied by an internal workspace gateway",
    )
...
class SearchRequest(BaseModel):
    ...
    notebook_id: str = Field(..., description="Notebook that bounds this search")
    source_ids: Optional[List[str]] = Field(
        None, description="Optional source allowlist inside the notebook; an empty list allows none"
    )
    excluded_source_ids: List[str] = Field(
        default_factory=list, description="Sources to exclude from this search"
    )
...
class AskRequest(BaseModel):
    ...
    notebook_id: str = Field(..., description="Notebook that bounds this question")
    source_ids: Optional[List[str]] = Field(
        None, description="Optional source allowlist inside the notebook; an empty list allows none"
    )
    excluded_source_ids: List[str] = Field(
        default_factory=list, description="Sources to exclude from this question"
    )
...
class SourceInsightUpdate(BaseModel):
    insight_type: Optional[str] = Field(None, min_length=1, max_length=200)
