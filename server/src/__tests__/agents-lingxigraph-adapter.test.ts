...
test('maps communication actions to argv without a shell or identity flag', () => {
  const cases: Array<[CommunicationAction, string[]]> = [
    ...
    [{ type: 'approval.request', conversationId: 'c1', kind: 'sensitive_or_destructive_action', summary: '删除文件', payload: { path: '/workspace/a' } }, ['approval', 'request', 'c1', 'sensitive_or_destructive_action', '删除文件', '--payload-json', '{"path":"/workspace/a"}']],
