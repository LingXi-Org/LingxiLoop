...
test('Agent OS has no direct network path to Open Notebook or SurrealDB', () => {
  for (const relative of composeFiles) {
    ...
    const notebook = serviceBlock(source, 'open-notebook', relative.includes('ci') ? 'fake-deepseek-provider' : 'wukongim')
...
test('Open Notebook shares LingxiLoop R2 credentials without exposing R2 publicly', () => {
  ...
  assert.match(serviceBlock(ci, 'open-notebook', 'fake-deepseek-provider'), /OPEN_NOTEBOOK_R2_ENABLED: "false"/)
})
