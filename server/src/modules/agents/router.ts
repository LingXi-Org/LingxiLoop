 ...
 * rather than defaulting everyone to one bucket.
 */
async function inferAgentGender(args: {
  ...
  const { createChatCompletion } = await import('../../llm.js')
    const r = await createChatCompletion({ purpose: 'agent-gender', companyId: args.tenant }, {
...
${systemPrompt.slice(0, 500) || '(none)'}` }],
  ...
  throw new Error('OpenAI returned an invalid gender classification')
}
...
api.post('/agents', async (req, res) => {
  ...
  await generateAndPersistAvatar({ agentId: newAgentId, tenant })
...
export async function generateAndPersistAvatar(args: {
  ...
  if (!env.OPENAI_IMAGE_MODEL) {
    throw new HttpError(501, 'image generation requires OPENAI_IMAGE_MODEL')
  }

  const styleHint = (a.system_prompt ?? '').slice(0, 500)
  // Gender inference is async — call before the visual signature so we pick
  // from the right bucket. Invalid provider output fails the operation.
  ...
  const r = await createImage({ purpose: 'agent-avatar', companyId: tenant, agentId: id }, {
