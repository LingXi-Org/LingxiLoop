...
api.post('/courses', safe(async (req, res) => {
  ...
  if (pulseCreated) {
    await sendTeacherAgentWelcome(courseId)
  }
  await ensureProjectNotebook(projectId, companyId)
...
api.patch('/courses/:id', safe(async (req, res) => {
  ...
  await syncProjectNotebookMetadata(manager.projectId)
...
api.delete('/courses/:id/members/:userId', safe(async (req, res) => {
  ...
  await Promise.all(changedChannels.map((channel) => wukongClient().upsertChannel({
...
api.post('/course-invitations/:token/accept', safe(async (req, res) => {
  ...
  if (result.joinedCompany) await seedMemberDms({ companyId: result.companyId, memberId: me })
