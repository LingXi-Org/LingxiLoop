...
import { ADMIN_ROLES } from '../../http/authorization.js'
...
api.post('/companies', async (req, res) => {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      const { rows: meRow } = await pool.query<{ display_name: string; avatar_url: string | null }>(
        `SELECT display_name, avatar_url FROM users WHERE id = $1`, [me],
      )
      if (!meRow[0]) throw new HttpError(401, 'session points to missing user')
      const displayName = meRow[0].display_name
      await pool.query(
        ...
        [me, displayName, displayName.charAt(0).toUpperCase(), meRow[0].avatar_url, id],
      )
...
api.delete('/companies/:id/members/:userId', safe(async (req, res) => {
  ...
  await Promise.all(channels.map((channel) => wukongClient().upsertChannel({
...
api.post('/companies/:id/invitations', safe(async (req, res) => {
  ...
  if (body.sendEmail === true && email) {
    ...
    if (!inviter || !company) throw new HttpError(500, 'inviter or company row missing')
 ...
 *  Then seeds direct chats without changing the
...
api.post('/invitations/:token/accept', safe(async (req, res) => {
  ...
  const userAvatar = userRow[0].avatar_url
  ...
  const inv = invRow[0]
  if (!inv) throw new HttpError(500, 'accepted invitation disappeared')
  ...
  await seedMemberDms({ companyId: inv.company_id, memberId: me })
  ...
  if (!company) throw new HttpError(500, 'accepted company membership missing')
  const ip = req.socket.remoteAddress ?? null
  const ua = (req.headers['user-agent'] as string | undefined) ?? null
  await audit({
    kind: 'invitation_accept',
    userId: me, companyId: inv.company_id, ip, userAgent: ua,
