...
async function requireBoardResource(
  scope: Awaited<ReturnType<typeof requireCompanyArtifactContext>>,
...
boardsRouter.get('/boards', safe(async (req, res) => {
  const scope = await requireCompanyArtifactContext(req, 'board:read')
  res.json(await boardApplication.boards(scope))
}))

boardsRouter.get('/cards/:id', safe(async (req, res) => {
  const scope = await requireCompanyArtifactContext(req, 'board:read')
  await requireBoardResource(scope, 'board:read', { type: 'board_card', id: String(req.params.id) })
...
boardsRouter.post('/boards', safe(async (req, res) => {
  const scope = await requireCompanyArtifactContext(req, 'board:write')
...
boardsRouter.get('/boards/:id', safe(async (req, res) => {
  const scope = await requireCompanyArtifactContext(req, 'board:read')
  await requireBoardResource(scope, 'board:read', { type: 'board', id: String(req.params.id) })
...
boardsRouter.patch('/boards/:id', safe(async (req, res) => {
  const scope = await requireCompanyArtifactContext(req, 'board:write')
  await requireBoardResource(scope, 'board:write', { type: 'board', id: String(req.params.id) })
