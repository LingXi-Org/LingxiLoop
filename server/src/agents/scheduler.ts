 ...
 * LINGXILOOP_AGENT_IDLE_MS without a wake, it sets the agent's status to
...
const WAKE_RETRY_DUE_KEY = 'lingxiloop:wake-retry:due'
const WAKE_RETRY_JOB_KEY = 'lingxiloop:wake-retry:jobs'
...
// message. During a lingxiloop-server crashloop, idle ticks accumulate in
...
//
// The fix is a hard cap on synthetic wakes per lingxiloop-server process
...
export async function consumeAgentTurnToken(agentId: string): Promise<boolean> {
  try {
    const key = `lingxiloop:turn-rate:${agentId}`
...
async function wakeOne(
  ...
  // BYOA agents run on a user-paired Computer (the `lingxiloop agent computer`
...
/** Multi-instance dedup: every lingxiloop-server replica subscribes to
...
async function claimAndWake(payload: MessageNewEvent): Promise<void> {
  const key = `lingxiloop:wake-claim:${payload.message.id}`
...
async function consumeSteerRateToken(agentId: string): Promise<boolean> {
  try {
    const key = `lingxiloop:steer-rate:${agentId}`
...
export async function _resetSteerRateForTests(agentId: string): Promise<void> {
  try { await redis.del(`lingxiloop:steer-rate:${agentId}`) } catch { /* ignore */ }
}
...
async function wake(payload: MessageNewEvent): Promise<void> {
  ...
  // Those affordances live in pod-agent + the `lingxiloop glance` tool +
...
//
// Debounce:
//   • vote events claim lingxiloop:poll-vote-wake-claim:<msg> with TTL
...
//     the wake (and can call `lingxiloop poll show` for newer state).
//   • close events claim a separate lingxiloop:poll-close-wake-claim:<msg>
...
async function handlePollUpdated(event: PollUpdatedEvent): Promise<void> {
  ...
  const claimKey = isClose
    ? `lingxiloop:poll-close-wake-claim:${event.messageId}`
    : `lingxiloop:poll-vote-wake-claim:${event.messageId}`
