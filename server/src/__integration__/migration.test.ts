...
test('an empty database reaches the latest schema once and repeated migration is a no-op', async () => {
  await withDatabase(async (database) => {
    assert.deepEqual(await migrateDatabase(database), ['0001_v1_baseline', '0002_remove_legacy_identity', '0003_agent_os_session_affinity', '0004_backfill_personal_owner_participants', '0005_lingxios_v2_reset', '0006_retire_legacy_eval'])
    ...
    assert.deepEqual(rows, [
      ...
      { version: 6, name: 'retire_legacy_eval' },
    ])
  })
})

test('legacy Eval tables retire transactionally while product evidence survives', async () => {
  await withMigrations(async (migrationsUrl, directory) => {
    for (const [source, name] of [
      [legacyCleanupUrl, '0002_remove_legacy_identity.sql'], [affinityUrl, '0003_agent_os_session_affinity.sql'],
      [personalOwnerUrl, '0004_backfill_personal_owner_participants.sql'], [lingxiOSResetUrl, '0005_lingxios_v2_reset.sql'],
    ] as const) await copyFile(source, join(directory, name))
    await withDatabase(async database => {
      await migrateDatabase(database, migrationsUrl)
      const before = await database.query('SELECT version,checksum FROM schema_migrations ORDER BY version')
      await database.query(`INSERT INTO eval_runs(id,suite_key,suite_name,version,status,score,pass_threshold,case_count,passed_cases,failed_cases,source,created_by)
        VALUES('retired','old','Old','1','pass',1,1,1,1,0,'inline','synthetic')`)
      await copyFile(new URL('../db/migrations/0006_retire_legacy_eval.sql', import.meta.url), join(directory, '0006_retire_legacy_eval.sql'))
      assert.deepEqual(await migrateDatabase(database, migrationsUrl), ['0006_retire_legacy_eval'])
      const { rows } = await database.query(`SELECT to_regclass('public.eval_runs') AS runs,
        to_regclass('public.eval_cases') AS cases,to_regclass('public.eval_stage_results') AS stages,
        to_regclass('public.evidence_records')::text AS evidence`)
      assert.deepEqual(rows, [{ runs: null, cases: null, stages: null, evidence: 'evidence_records' }])
      assert.deepEqual(await migrateDatabase(database, migrationsUrl), [])
      assert.deepEqual((await database.query('SELECT version,checksum FROM schema_migrations WHERE version<=5 ORDER BY version')).rows, before.rows)
    })
  })
})
  ...
  await withDatabase(async (database, connectionString) => {
    ...
    try {
      ...
      assert.deepEqual(results.map((result) => [...result]).sort((a, b) => b.length - a.length), [
        ['0001_v1_baseline', '0002_remove_legacy_identity', '0003_agent_os_session_affinity', '0004_backfill_personal_owner_participants', '0005_lingxios_v2_reset', '0006_retire_legacy_eval'],
        [],
      ])
