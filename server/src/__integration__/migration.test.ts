...
test('an empty database reaches the latest schema once and repeated migration is a no-op', async () => {
  await withDatabase(async (database) => {
    ...
    assert.deepEqual(rows, [
      ...
      { version: 29, name: 'lingxios_4_0_1' },
      { version: 30, name: 'lingxios_5_0_0' },
    ])
...
test('collaboration upgrade preserves managed IDs, custom rooms and leaders while expanding default teams', async () => {
  await withDatabase(async database => {
    ...
    await database.query(`DELETE FROM schema_migrations WHERE version>=27;
      UPDATE lingxios_installation SET runtime_version='3.3.2',protocol_version=11;
    ...
    assert.deepEqual(await migrateDatabase(database),['0027_learning_collaboration','0028_lingxios_3_3_4','0029_lingxios_4_0_1','0030_lingxios_5_0_0'])
...
test('earlier 3.2 versions upgrade only runtime registration and preserve existing data', async () => {
  for (const version of ['3.2.4','3.2.6','3.2.8','3.2.9','3.2.10','3.2.11','3.2.12']) await withDatabase(async database => {
    ...
    assert.deepEqual(await migrateDatabase(database),[...version === '3.2.4' ? ['0015_lingxios_3_2_6'] : [],
      ...
      ...version !== '3.2.12' ? ['0022_lingxios_3_2_12'] : [],'0023_lingxios_3_2_13','0024_lingxios_3_3_0','0025_lingxios_3_3_1','0026_lingxios_3_3_2','0027_learning_collaboration','0028_lingxios_3_3_4','0029_lingxios_4_0_1','0030_lingxios_5_0_0'])
    assert.deepEqual(await migrateDatabase(database),[])
    await assertMigrationsCurrent(database)
...
test('read receipt upgrade preserves human receipts and accepts only same-tenant participants', async () => {
  await withDatabase(async database => {
    ...
    await database.query(`ALTER TABLE im_read_receipt_advances DROP CONSTRAINT im_read_receipt_reader_company_fkey,
      ...
      UPDATE lingxios_installation SET runtime_version='3.3.0',protocol_version=11;
...
test('3.2.13 upgrades the workspace schema transactionally and repeats without changing product data', async () => {
  await withDatabase(async database => {
    ...
    assert.deepEqual(await migrateDatabase(database), ['0024_lingxios_3_3_0','0025_lingxios_3_3_1','0026_lingxios_3_3_2','0027_learning_collaboration','0028_lingxios_3_3_4','0029_lingxios_4_0_1','0030_lingxios_5_0_0'])
    assert.deepEqual(await migrateDatabase(database), [])
    await assertMigrationsCurrent(database)
...
for (const [version, pending] of [['3.3.0', 25], ['3.3.1', 26], ['3.3.2', 28], ['3.3.4', 29]] as const) test(`${version} registers the runtime fixes without altering sessions or accepting an unverified schema`, async () => {
  await withDatabase(async database => {
    await migrateDatabase(database)
    await database.query('DELETE FROM schema_migrations WHERE version >= $1', [pending])
    await database.query('UPDATE lingxios_installation SET runtime_version=$1,protocol_version=11', [version])
    ...
    await database.query("UPDATE lingxios_installation SET schema_sha256=repeat('0',64)")
    await assert.rejects(migrateDatabase(database), /migration (?:25_lingxios_3_3_1|26_lingxios_3_3_2|28_lingxios_3_3_4|29_lingxios_4_0_1) failed/)
    ...
    await assert.rejects(assertMigrationsCurrent(database), /run `npm run db:migrate`/)
    assert.deepEqual(await migrateDatabase(database), [...pending === 25 ? ['0025_lingxios_3_3_1'] : [], ...pending <= 26 ? ['0026_lingxios_3_3_2','0027_learning_collaboration'] : [], ...pending <= 28 ? ['0028_lingxios_3_3_4'] : [],'0029_lingxios_4_0_1','0030_lingxios_5_0_0'])
    assert.deepEqual(await migrateDatabase(database), [])
    await assertMigrationsCurrent(database)
    assert.deepEqual((await database.query('SELECT runtime_version,schema_version,protocol_version,schema_sha256 FROM lingxios_installation')).rows,
      [{ runtime_version: '5.0.0-rc.2', schema_version: 11, protocol_version: 13, schema_sha256: installation[0].schema_sha256 }])
...
test('protocol 12 cutover refuses leased work and preserves the old installation atomically', async () => {
  await withDatabase(async database => {
    await migrateDatabase(database)
    await database.query(`DELETE FROM schema_migrations WHERE version>=29;
      UPDATE lingxios_installation SET runtime_version='3.3.4',protocol_version=11;
      INSERT INTO lingxios.agent_work_items(id,tenant_id,agent_id,session_id,kind,lane,trigger_ref,status,started_at,lease_expires_at)
        VALUES('leased-cutover','tenant','agent','session','turn','interactive','message','leased',NOW(),NOW()+INTERVAL '1 minute')`)
    await assert.rejects(migrateDatabase(database), /migration 29_lingxios_4_0_1 failed/)
    assert.deepEqual((await database.query('SELECT runtime_version,protocol_version FROM lingxios_installation')).rows,
      [{ runtime_version: '3.3.4', protocol_version: 11 }])
    await database.query("UPDATE lingxios.agent_work_items SET status='cancelled' WHERE id='leased-cutover'")
    assert.deepEqual(await migrateDatabase(database), ['0029_lingxios_4_0_1','0030_lingxios_5_0_0'])
    assert.deepEqual(await migrateDatabase(database), [])
  })
})
...
test('protocol 13 decision cutover refuses active leases and preserves the installation', async () => {
  await withDatabase(async database => {
    await migrateDatabase(database)
    await database.query(`DELETE FROM schema_migrations WHERE version=30;
      UPDATE lingxios_installation SET runtime_version='4.0.1',protocol_version=12;
      INSERT INTO lingxios.agent_work_items(id,tenant_id,agent_id,session_id,kind,lane,trigger_ref,status,started_at,lease_expires_at)
        VALUES('decision-cutover','tenant','agent','session','turn','interactive','message','leased',NOW(),NOW()+INTERVAL '1 minute')`)
    await assert.rejects(migrateDatabase(database), /migration 30_lingxios_5_0_0 failed/)
    assert.deepEqual((await database.query('SELECT runtime_version,protocol_version FROM lingxios_installation')).rows,
      [{ runtime_version: '4.0.1', protocol_version: 12 }])
    await database.query("UPDATE lingxios.agent_work_items SET status='cancelled' WHERE id='decision-cutover'")
    assert.deepEqual(await migrateDatabase(database), ['0030_lingxios_5_0_0'])
    await assertMigrationsCurrent(database)
  })
})
...
test('concurrent migrators serialize and apply each migration once', async () => {
  await withDatabase(async (database, connectionString) => {
    ...
    assert.deepEqual(rows, [{ count: 30 }])
    } finally {
      await second.end()
    }
  })
