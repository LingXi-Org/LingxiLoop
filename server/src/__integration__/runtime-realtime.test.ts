...
test('shares fenced drafts across nodes, isolates identities, recovers lost cache and pubsub, and releases subscriptions',
  ...
  try {
    ...
    assert.equal(await a.read(identity, signal), null)
    assert.equal(await a.update(identity, 'reconnected', 2, { ...delta, seq: 4 }, undefined, signal), 'stale')
    assert.equal(await a.update(identity, 'reconnected', 2, { ...reset, seq: 4 }, undefined, signal), 'applied')
    assert.equal(await a.update(identity, 'reconnected', 2, { ...delta, seq: 5, text: '恢复正文' }, undefined, signal), 'applied')
    await b.clear(identity, 'new-owner', 2, signal)
    assert.equal((await b.read(identity, signal))?.draft, '恢复正文')
    assert.equal(await b.update(identity, 'new-owner', 2, { ...reset, seq: 6 }, undefined, signal), 'stale')
