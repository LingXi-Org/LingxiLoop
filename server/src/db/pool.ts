...
export const pool = new Pool({
  ...
  // transactions holding a slot open doing nothing. Boot migrations and
  // CONCURRENTLY index builds legitimately run longer and disable these on their
  // own session (see ensureSchema -> `SET statement_timeout = 0`).
