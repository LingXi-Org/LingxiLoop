...
npm run eval -- run --env .env.local --suite suites/black-box-smoke.v1.json --dataset datasets/smoke.v1.json --revision YOUR_COMMIT --baseline smoke-reference-v1
```
...
npm run eval -- run --env .env.local --suite suites/black-box-smoke.v1.json --dataset datasets/smoke.v1.json --revision YOUR_COMMIT --baseline-file baselines/smoke-reference-v1.json
```
...
```sh
npm run eval -- enqueue --env .env.local --suite suites/black-box-smoke.v1.json --dataset datasets/smoke.v1.json --revision YOUR_COMMIT --baseline smoke-reference-v1
...
`.github/workflows/black-box-eval.yml` is a manual/reusable release gate requiring a reviewed baseline file, two secrets and four pricing variables. It fails on blocked gates and uploads sanitized JSON/Markdown/span reports. A publishing workflow consuming it must depend on its success. It stays separate from AgentOS CI until an external adapter exists; its score is not AgentOS quality. Ordinary CI runs infrastructure checks when the independent package changes.
...
Artifacts under `eval/.state/reports/` include aggregate/sample JSON, Markdown and JSONL spans. They store output hashes, synthetic IDs and bounded failure codes, never prompts, answers, keys or provider error bodies. Private SQLite under `eval/.state/` retains dataset inputs for replay: protect it with host ACLs, back it up as sensitive data and never upload it publicly. `--db` and `--out` select storage. No visualization frontend is implemented this phase.
