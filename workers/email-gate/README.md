# lingxiloop-email-gate

Cloudflare Email Worker that fronts the agents' inbox.

## What it does

Cloudflare Email Routing → this worker → lingxiloop-server `/webhooks/email/inbound`.
...
4. HMAC-signs the payload with `EMAIL_INBOUND_HMAC_SECRET` and POSTs to `LINGXILOOP_INBOUND_URL`.
...
1. Pick the zone for your `EMAIL_DOMAIN` (e.g. `mail.loop.example.com`).
2. **Email → Email Routing → Get started** (this also adds the right MX records automatically).
3. Under **Catch-all address**, choose **Send to a Worker** → `lingxiloop-email-gate`.
...
1. Tunnel your local lingxiloop-server: `cloudflared tunnel --url http://localhost:5181`.
2. Set the worker's `LINGXILOOP_INBOUND_URL` to the tunnel URL + `/webhooks/email/inbound`:
   ```toml
   [vars]
   LINGXILOOP_INBOUND_URL = "https://your-tunnel.example/webhooks/email/inbound"
   ```
...
```bash
BODY='{"messageId":"test-123@local","from":"Test Sender <sender@example.com>","to":["aurora.personal@mail.loop.example.com"],"subject":"hi","text":"yo"}'
...
curl -X POST http://localhost:5181/webhooks/email/inbound \
     ...
     -H "x-lingxiloop-signature: sha256=$SIG" \
     --data "$BODY"
```
