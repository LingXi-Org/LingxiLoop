...
1. Treat `VITE_TURNSTILE_SITE_KEY` as public build configuration. Keep `TURNSTILE_SECRET_KEY`, `ALIYUN_OTP_EMAIL_PASSWORD`, and `ALIYUN_SUPPORT_EMAIL_PASSWORD` as Worker secrets set only with Wrangler secret commands; use JSON `wrangler secret bulk` for the Alibaba passwords so Windows line-oriented input cannot append a newline. Never place them in Wrangler vars, GitHub variables, logs, patches, or repository files. OTP and reset mail authenticate as `no-reply@lingxilearn.cn`; the reserved feedback mailbox is `support@lingxilearn.cn`.
