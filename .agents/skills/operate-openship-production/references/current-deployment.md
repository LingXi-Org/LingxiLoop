...
At 13:42, Worker tail proved the original line-oriented secret upload caused Alibaba SMTP `526 Authentication failure`; rewriting both mailbox secrets with JSON `wrangler secret bulk` produced version `8c8933b2-4426-4a5d-a678-03a529bb5496` and a real OTP request completed without exceptions. PR `#14` then committed the SMTP implementation and merged as `a2e032393ac6692134c9b8cc2c35009e495a3fed`. GitHub Actions run `33720582271` passed and promoted tagged Worker version `0299d333-37b2-4b61-99b1-89e673cf3503` at 100% traffic. SMTP accepted the post-CD OTP but the QQ recipient still reported no delivery; the sender mailbox had no bounce, while an Alibaba-delivered copy proved the generated message lacked `Date` and `Message-ID`. A follow-up adds both standard headers. Public health endpoints remained `200`, and OpenShip's watcher remained enabled with 16/16 services healthy.
