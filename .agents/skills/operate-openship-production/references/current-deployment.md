...
Snapshot: 2026-09-03 11:46 China Standard Time, checked through OpenShip MCP, GitHub CLI, HTTP/TLS/private-port probes, Uptime SQLite status, and host inspection. Re-read live state before every operation.
...
All six projects run manifest commit `ad9a7f2e8ba3397943babcde1b802edb48e03941`. Server, AgentOS, WuKongIM, and Open Notebook use immutable tag `b42fef160fe697d46a8818e054f945d1f80953f7`; Gateway uses the later auth-routing fix `3b0069a17ec0a969d85301b13aba386832001cdc`. A release must contain all five valid pins, but unchanged components keep their previous immutable tags.

| Project | ID | Host | Compose | Active deployment |
| --- | --- | --- | --- | --- |
| `lingxiloop-core-state` | `proj_khiExWfh7Vsj72VO` | A | `deploy/openship/core-state.yml` | `dep_C82hkdGyP9iHCgOi` |
| `lingxiloop-app-a` | `proj_5uz48XlBkfJQeNC8` | A | `deploy/openship/app-a.yml` | `dep_IRPncv9B85bCcaE0` |
| `lingxiloop-agent-os-a` | `proj_29J2mM47umuIfaDK` | A | `deploy/openship/agent-os.yml` | `dep_y8h-s-eQpH141a9b` |
| `lingxiloop-app-b` | `proj_IsMy2bWVzEZ7JKEf` | B | `deploy/openship/app-b.yml` | `dep_R2UaNNG8Win4TkSX` |
| `lingxiloop-knowledge-agent` | `proj_frnQUaoQY37ejzL-` | B | `deploy/openship/knowledge-agent.yml` | `dep_ywPSJ1i8m2tzbDjx` |
| `lingxiloop-agent-os-b` | `proj_CVkF0rOULikADQ-7` | B | `deploy/openship/agent-os.yml` | `dep_1nt0A5nbtNX0mdU9` |
...
| Gateway | `svc_q7ZcH8px3jsB9qnY` | `lingxiloop-gateway:3b0069a...` | `127.0.0.1:8080` | none |
...
- Public `https://loop.lingxilearn.cn/api/auth/get-session` returns JSON HTTP 200 through the Gateway; the running Gateway container contains the `/api/auth/` proxy and uses image `3b0069a...`.
