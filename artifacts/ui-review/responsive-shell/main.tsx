...
localStorage.setItem('lingxiloop-theme', new URLSearchParams(location.search).get('theme') === 'light' ? 'light' : 'dark')
...
function ResponsiveShellReview() {
  const [paletteIndex, setPaletteIndex] = useState(0)
  const palette = SHELL_PALETTES[paletteIndex]!
  return (
    <div className="responsive-shell-review" data-shell-palette={palette.id}>
      <div className="responsive-shell-toolbar" aria-label="响应式外壳审查工具栏">
        <Button
          type="button"
          variant="outline"
          className="h-9 gap-2 rounded-lg bg-card/95 px-3 text-xs shadow-lg backdrop-blur"
          onClick={() => setPaletteIndex((current) => (current + 1) % SHELL_PALETTES.length)}
          aria-label={`切换界面配色，当前为${palette.label}`}
        >
          <span className="shell-palette-swatch" aria-hidden />
          界面配色：{palette.label} · {paletteIndex + 1}/{SHELL_PALETTES.length}
        </Button>
        <ThemeToggle className="size-9 border border-border bg-card/95 p-0 text-foreground shadow-lg backdrop-blur hover:bg-accent" />
      </div>
      <DesktopApp />
    </div>
  )
...
createRoot(document.getElementById('root')!).render(
  <AppThemeProvider>
    <TooltipProvider>
      <GlobalInteractionProvider><ResponsiveShellReview /></GlobalInteractionProvider>
    </TooltipProvider>
  </AppThemeProvider>,
)
