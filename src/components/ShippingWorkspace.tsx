...
export function ShippingWorkspace({ compact = false }: { compact?: boolean }) {
  ...
  if (loading && !overview) return <Centered title="正在加载运输循环..." />
  if (!overview) return <Centered title="运输工作区不可用" detail={error ?? undefined} retry={() => void load(true)} />
    ...
    <div className={cn('h-full min-h-0 bg-[#F7FAFC]', compact ? 'relative overflow-hidden' : 'grid grid-cols-[300px_minmax(0,1fr)]')}>
        ...
        <aside className={cn('h-full overflow-y-auto border-r border-ink-100 bg-cloud', compact && 'absolute inset-0 border-r-0')}>
          <div className="sticky top-0 z-10 border-b border-ink-100 bg-cloud/95 px-4 py-4 backdrop-blur">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-[18px] font-semibold text-ink-900">交付</h1>
                <p className="mt-0.5 text-[11px] text-ink-400">证据、发布、重读、学习。</p>
              </div>
              <button onClick={() => setCreating(true)} className="grid h-9 w-9 place-items-center rounded-xl bg-skype text-white shadow-soft" aria-label="新功能" title="新的运输功能"><IPlus className="h-4 w-4" /></button>
            </div>
          </div>
          ...
          {overview.dueReadbacks.length > 0 && (
            <div className="mx-3 mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5">
              <div className="text-[11px] font-bold text-amber-800">{overview.dueReadbacks.length} 生产回读{overview.dueReadbacks.length === 1 ? '' : 's'} 由于</div>
              <div className="mt-1 text-[10px] text-amber-700">在生产证据返回之前不会获悉发布。</div>
            </div>
          )}
          <div className="p-2">
            {overview.features.map((feature) => {
              ...
              return (
                <button key={feature.id} onClick={() => void select(feature.id)} className={cn('mb-1 w-full rounded-xl px-3 py-3 text-left transition', selectedId === feature.id ? 'bg-sky2-50 ring-1 ring-sky2-200' : 'hover:bg-ink-50')}>
                  ...
                  <div className="mt-1.5 flex justify-between text-[10px] text-ink-400"><span>{feature.passedSquares}/{feature.requiredSquares} 所需的方块</span><span>{feature.riskLevel} 风险</span></div>
                </button>
              )
            })}
            {overview.features.length === 0 && (
              <div className="px-6 py-16 text-center"><IShip className="mx-auto h-9 w-9 text-ink-300" /><div className="mt-3 text-[13px] font-semibold text-ink-700">尚未有运输合同</div><p className="mt-1 text-[11px] leading-relaxed text-ink-400">从问题、期望的结果和构建者开始。 LingxiLoop 将播种不可协商的证据方块。</p></div>
            )}
          </div>
        </aside>
      )}
      {showDetail && (
        <main className={cn('h-full min-w-0 overflow-y-auto', compact && 'absolute inset-0 bg-[#F7FAFC]')}>
          {compact && <button onClick={() => void select(null)} className="sticky top-0 z-20 flex h-11 w-full items-center gap-1 border-b border-ink-100 bg-cloud/95 px-3 text-[12px] font-semibold text-skype-deep backdrop-blur"><IBack className="h-4 w-4" /> 所有功能</button>}
          {loadingFeatureId === selectedId && !detail ? <Centered title="签订运输合同..." /> : detail ? <FeatureDetail feature={detail} /> : <Centered title="选择一个功能以打开其运输合同" />}
        </main>
      )}
    </div>
  )
...
function CreateFeature({ onClose }: { onClose: () => void }) {
  ...
  return <form onSubmit={(e) => void submit(e)} className="m-3 rounded-2xl border border-sky2-200 bg-sky2-50/60 p-3 shadow-soft">
    <div className="text-[12px] font-bold text-ink-800">新运输合同</div>
    <input className={cn(fieldClass(), 'mt-2')} value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="专题标题" autoFocus />
    <textarea className={cn(fieldClass(), 'mt-2 min-h-16')} value={form.problem} onChange={(e) => setForm({ ...form, problem: e.target.value })} placeholder="我们要解决什么问题？" />
    <textarea className={cn(fieldClass(), 'mt-2 min-h-16')} value={form.desiredOutcome} onChange={(e) => setForm({ ...form, desiredOutcome: e.target.value })} placeholder="哪些可观察到的结果应该改变？" />
    <textarea className={cn(fieldClass(), 'mt-2 min-h-16')} value={form.contractSummary} onChange={(e) => setForm({ ...form, contractSummary: e.target.value })} placeholder="范围、限制、非目标" />
    <div className="mt-2 text-[10px] font-bold uppercase tracking-wide text-ink-400">建设者</div>
    ...
    <div className="mt-3 flex justify-end gap-2"><Button onClick={onClose}>取消</Button><Button type="submit" tone="primary" disabled={saving || !form.title.trim()}>创建合约</Button></div>
  </form>
}
  ...
  return <div className="mx-auto max-w-[1100px] px-4 py-5 md:px-7 md:py-7">
    <header className="rounded-2xl border border-ink-100 bg-white p-5 shadow-soft">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Pill tone="blue">{STATUS_LABEL[feature.status]}</Pill><Pill tone={feature.riskLevel === 'critical' || feature.riskLevel === 'high' ? 'warn' : 'neutral'}>{feature.riskLevel} 风险</Pill></div><h1 className="mt-2 text-[22px] font-semibold tracking-tight text-ink-900">{feature.title}</h1><p className="mt-1 text-[11px] text-ink-400">{passed}/{required.length} 所需证据方块已通过 · 已更新 {new Date(feature.updatedAt).toLocaleString()}</p></div>
        {next && feature.status !== 'ready' && feature.status !== 'releasing' && <Button tone="primary" disabled={saving} onClick={() => runAction(transition(next), () => event('shipping_feature_transitioned', { from: feature.status, to: next }))}>移至 {STATUS_LABEL[next]}</Button>}
      </div>
...
function ContractSection({ feature }: { feature: ShippingFeatureDetail }) {
  ...
  return <Section eyebrow="01·定义" title="功能合约" action={<Button tone="primary" disabled={saving} onClick={() => runAction(update(form), () => event('shipping_contract_saved'))}>保存合同</Button>}>
    <div className="grid gap-3 md:grid-cols-2"><label className="text-[11px] font-semibold text-ink-600">标题<input className={cn(fieldClass(), 'mt-1')} value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} /></label><label className="text-[11px] font-semibold text-ink-600">释放目标<input className={cn(fieldClass(), 'mt-1')} value={form.releaseTarget} onChange={(e) => setForm({ ...form, releaseTarget: e.target.value })} placeholder="e.g. 2026-W31 / v0.2" /></label></div>
...
function InvariantsSection({ feature }: { feature: ShippingFeatureDetail }) {
  ...
  return <Section eyebrow="02·约束" title="不变量" action={<div className="text-[10px] text-ink-400">必须保持真实的是什么</div>}>
    ...
    <form className="mt-3 flex flex-col gap-2 md:flex-row" onSubmit={(e) => { e.preventDefault(); if (!title.trim()) return; runAction(mutate((id) => api.createShippingInvariant(id, { title, kind })), () => { setTitle(''); event('shipping_invariant_created', { kind }) }) }}><input className={cn(fieldClass(), 'flex-1')} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="添加一个不变量：例如重试绝不会重复收费" /><select className={cn(fieldClass(), 'md:w-40')} value={kind} onChange={(e) => setKind(e.target.value)}>{['behavior','architecture','data','security','performance','ux','operability'].map((v) => <option key={v}>{v}</option>)}</select><Button type="submit" disabled={!title.trim()}><IPlus className="mr-1 inline h-3 w-3" /> 添加</Button></form>
  </Section>
}
...
function VerificationsSection({ feature }: { feature: ShippingFeatureDetail }) {
  ...
  return <Section eyebrow="03 · 验证" title="独立证据方块" action={<div className="text-[10px] text-ink-400">构建者≠验证者·需要证据</div>}>
    ...
    <form className="mt-3 flex flex-col gap-2 md:flex-row" onSubmit={(e) => { e.preventDefault(); if (!title.trim()) return; runAction(mutate((id) => api.createShippingVerification(id, { title, method })), () => { setTitle(''); event('shipping_square_created', { method }) }) }}><input className={cn(fieldClass(), 'flex-1')} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="添加验证方块" /><select className={cn(fieldClass(), 'md:w-48')} value={method} onChange={(e) => setMethod(e.target.value)}>{['user_path','property','trace','data_reconciliation','design_qa','security','performance','release_note'].map((v) => <option key={v}>{v.replaceAll('_', ' ')}</option>)}</select><Button type="submit" disabled={!title.trim()}>添加方块</Button></form>
  </Section>
}
...
function VerificationCard({ feature, square }: { feature: ShippingFeatureDetail; square: ShippingVerification }) {
  ...
  return <details className="group rounded-xl border border-ink-100 bg-[#FAFCFD]" open={square.status === 'failed'}><summary className="flex cursor-pointer list-none items-center gap-3 px-3 py-3"><span className={cn('grid h-6 w-6 place-items-center rounded-full border text-[11px] font-bold', square.status === 'passed' ? 'border-emerald-300 bg-emerald-50 text-emerald-700' : square.status === 'failed' ? 'border-coral/30 bg-coral-soft text-coral-deep' : 'border-ink-200 bg-white text-ink-400')}>{square.status === 'passed' ? '✓' : square.status === 'failed' ? '!' : '·'}</span><div className="min-w-0 flex-1"><div className="truncate text-[12px] font-semibold text-ink-800">{square.title}</div><div className="mt-0.5 text-[10px] text-ink-400">{square.method.replaceAll('_', ' ')} · {square.required ? "需要" : "可选"} · {square.ownerId ? `owner @${square.ownerId}` : "无主"}</div></div><Pill tone={tone}>{square.status}</Pill></summary><div className="border-t border-ink-100 px-3 py-3"><div className="grid gap-2 md:grid-cols-[220px_1fr]"><label className="text-[10px] font-bold uppercase tracking-wide text-ink-400">独立所有者<select className={cn(fieldClass(), 'mt-1')} value={ownerId} onChange={(e) => setOwnerId(e.target.value)}><option value="">未分配</option>{participants.map((p) => <option value={p.id} key={p.id}>{p.name} (@{p.id})</option>)}</select></label><label className="text-[10px] font-bold uppercase tracking-wide text-ink-400">证据/链接/观察<input className={cn(fieldClass(), 'mt-1')} value={evidence} onChange={(e) => setEvidence(e.target.value)} placeholder="你检查了什么并证明了什么？" /></label></div><label className="mt-2 block text-[10px] font-bold uppercase tracking-wide text-ink-400">注释<input className={cn(fieldClass(), 'mt-1')} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="放弃时必填" /></label><div className="mt-3 flex flex-wrap gap-2"><Button onClick={() => runAction(patch())}>保存所有者</Button><Button tone="primary" onClick={() => runAction(patch('running'))}>开始</Button><Button tone="success" disabled={!evidence.trim()} onClick={() => runAction(patch('passed'), () => event('shipping_square_completed', { method: square.method, result: 'passed' }))}>有证据通过</Button><Button tone="danger" disabled={!evidence.trim()} onClick={() => runAction(patch('failed'), () => event('shipping_square_completed', { method: square.method, result: 'failed' }))}>证据不足</Button><Button disabled={!notes.trim()} onClick={() => runAction(patch('waived'))}>放弃</Button></div>{square.evidence.length > 0 && <pre className="mt-3 max-h-28 overflow-auto rounded-lg bg-ink-900 p-2 text-[10px] text-white/80">{JSON.stringify(square.evidence, null, 2)}</pre>}</div></details>
}
...
function ReleaseSection({ feature }: { feature: ShippingFeatureDetail }) {
  ...
  return <Section eyebrow="04·发布" title="点火、烟雾和生产回读" action={<Button onClick={() => setShow((v) => !v)}>{show ? "关闭计划器" : "计划发布"}</Button>}>
    {show && <div className="mb-4 rounded-xl border border-sky2-200 bg-sky2-50/50 p-3"><div className="grid gap-2 md:grid-cols-3"><select className={fieldClass()} value={form.environment} onChange={(e) => setForm({ ...form, environment: e.target.value })}>{['staging','canary','production'].map((v) => <option key={v}>{v}</option>)}</select><input className={fieldClass()} value={form.version} onChange={(e) => setForm({ ...form, version: e.target.value })} placeholder="版本" /><input className={fieldClass()} value={form.commitSha} onChange={(e) => setForm({ ...form, commitSha: e.target.value })} placeholder="提交 SHA" /></div><textarea className={cn(fieldClass(), 'mt-2 min-h-20')} value={form.releaseNotes} onChange={(e) => setForm({ ...form, releaseNotes: e.target.value })} placeholder="发行说明：用户可见的更改和已知差距" /><textarea className={cn(fieldClass(), 'mt-2 min-h-20')} value={form.rollbackPlan} onChange={(e) => setForm({ ...form, rollbackPlan: e.target.value })} placeholder="回滚计划：确切的触发器和命令/路径" /><input className={cn(fieldClass(), 'mt-2')} value={form.baseline} onChange={(e) => setForm({ ...form, baseline: e.target.value })} placeholder="生产基线指标（生产批准所需）" /><div className="mt-2 flex justify-end"><Button tone="primary" onClick={() => runAction(create())}>创建释放门</Button></div></div>}
    <div className="space-y-2">{feature.releases.map((release) => <ReleaseCard key={release.id} featureId={feature.id} release={release} />)}{feature.releases.length === 0 && <p className="text-[11px] text-ink-400">尚未发布。生产前证明分期；生产批准还需要发行说明、回滚计划和基线。</p>}</div>
  </Section>
}
...
function ReleaseCard({ featureId, release }: { featureId: string; release: ShippingRelease }) {
  ...
  return <details className="rounded-xl border border-ink-100 bg-[#FAFCFD]"><summary className="flex cursor-pointer list-none items-center gap-3 px-3 py-3"><Pill tone={release.status === 'succeeded' ? 'good' : release.status === 'failed' || release.status === 'rolled_back' ? 'bad' : release.status === 'running' ? 'blue' : 'warn'}>{release.environment}</Pill><div className="min-w-0 flex-1"><div className="text-[12px] font-semibold text-ink-800">{release.version || release.commitSha || 'Unversioned release'}</div><div className="mt-0.5 text-[10px] text-ink-400">{release.status} · 回读 {release.readbackStatus}{release.readbackDueAt ? ` · due ${new Date(release.readbackDueAt).toLocaleString()}` : ''}</div></div></summary><div className="border-t border-ink-100 p-3"><div className="grid gap-2 text-[11px] text-ink-600 md:grid-cols-2"><div><b>发行说明</b><p className="mt-1 whitespace-pre-wrap">{release.releaseNotes || '—'}</p></div><div><b>回滚计划</b><p className="mt-1 whitespace-pre-wrap">{release.rollbackPlan || '—'}</p></div></div>{actions.length > 0 && <><input className={cn(fieldClass(), 'mt-3')} value={evidence} onChange={(e) => setEvidence(e.target.value)} placeholder={actions.includes('approve') || actions.includes('start') ? "可选注释" : "冒烟/回读证据或回滚原因"} /><div className="mt-2 flex flex-wrap gap-2">{actions.map((name) => <Button key={name} tone={name.includes('fail') || name === 'rollback' ? 'danger' : name.includes('pass') || name === 'succeed' ? 'success' : 'primary'} disabled={['succeed','fail','readback_pass','readback_fail','rollback'].includes(name) && !evidence.trim()} onClick={() => runAction(action(name))}>{name.replaceAll('_', ' ')}</Button>)}</div></>}</div></details>
}
...
function LearningSection({ feature }: { feature: ShippingFeatureDetail }) {
  ...
  return <Section eyebrow="05·学习" title="摩擦和回归记忆">
    <div className="grid gap-4 md:grid-cols-2">
      <div>
        <div className="text-[11px] font-bold text-ink-700">摩擦收件箱</div>
        <div className="mt-2 space-y-2">
          {feature.frictions.map((item) => <div key={item.id} className="rounded-xl border border-ink-100 p-3">
            ...
            {!['resolved','dismissed'].includes(item.status) && <Button className="mt-2" onClick={() => runAction(resolveFriction(item.id))}>解决</Button>}
          </div>)}
        </div>
        <form className="mt-2 flex gap-2" onSubmit={(e) => { e.preventDefault(); if (!friction.trim()) return; runAction(captureFriction(), () => { setFriction(''); event('shipping_friction_reported') }) }}>
          <input className={fieldClass()} value={friction} onChange={(e) => setFriction(e.target.value)} placeholder="哪里的工作感觉比应有的更辛苦？" />
          <Button type="submit">捕获</Button>
        </form>
      </div>
      <div>
        <div className="text-[11px] font-bold text-ink-700">回归资产</div>
        <div className="mt-2 space-y-2">
        ...
        <form className="mt-2 flex gap-2" onSubmit={(e) => { e.preventDefault(); if (!regression.trim()) return; runAction(mutate((id) => api.createShippingRegression(id, { title: regression, kind: 'manual_replay', expected: 'Previously verified behavior remains true' })), () => { setRegression(''); event('shipping_regression_created') }) }}>
          <input className={fieldClass()} value={regression} onChange={(e) => setRegression(e.target.value)} placeholder="将课程变成可重播的检查" />
          <Button type="submit">哈登</Button>
        </form>
      </div>
    </div>
  </Section>
