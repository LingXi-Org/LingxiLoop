...
import { RATIO_CLASS_MAP, getFitClass } from "../shared/media/aspect-ratio";
import { openSafeNavigationHref } from "../shared/media/safe-navigation";
import { sanitizeHref } from "../shared/media/sanitize-href";
  ...
  return (
    ...
    >
      <a
        ...
        aria-label={`打开来源：${title ?? domain ?? rawHref}`}
        className={cn(
          "group @container relative isolate flex w-full min-w-0 flex-col overflow-hidden rounded-xl",
          "border-border bg-card border text-sm shadow-xs",
          sanitizedHref && "cursor-pointer focus-visible:ring-ring focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none",
        )}
        onClick={sanitizedHref && onNavigate ? (event) => { event.preventDefault(); handleClick(); } : undefined}
      >
            ...
            >
              <img
                ...
                className={cn(
                  ...
                  getFitClass(fit),
                  "object-center transition-transform duration-200 group-hover:scale-[1.01] motion-reduce:transition-none motion-reduce:transform-none",
                )}
              />
            </div>
          )}
