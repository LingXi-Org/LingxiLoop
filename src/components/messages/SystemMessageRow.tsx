...
export function SystemRow({ msg, delay = 0, animate = true, openMaus = false }: { msg: { body: string }; delay?: number; animate?: boolean; openMaus?: boolean }) {
  ...
  if (payload.kind === 'notice' && typeof payload.text === 'string') return <MessagePrimitive.Root {...rootProps} data-message-shell="system">
    <div data-message-surface="status" data-card-status="failed" className="flex max-w-[min(100%,540px)] items-start gap-2 rounded-2xl border border-destructive/20 bg-destructive/10 px-3 py-1.5 text-[11.5px] text-destructive">
  ...
  if (payload.kind === 'calendar_event') {
    ...
    return <MessagePrimitive.Root {...rootProps} data-message-shell="system">
      <div data-message-surface="status" className="flex max-w-[min(100%,540px)] items-center gap-2 rounded-2xl border border-primary/20 bg-primary/10 px-3 py-1.5 text-[11.5px] text-primary">
  ...
  return <MessagePrimitive.Root {...rootProps} data-message-shell="system">
    <div className="flex flex-wrap items-center justify-center gap-1.5 text-[11.5px] italic text-muted-foreground">
