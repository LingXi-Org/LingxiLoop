...
export function ReadReceiptStatus() {
  ...
  if (message.failed) return <span className="mt-1 text-[10px] font-medium text-destructive">失败</span>
  if (message.pending || !message.sequence) return <span className="mt-1 text-[10px] text-muted-foreground">发送中</span>

  const confirmations = confirmationsForMessage(receipts, message.sequence, meId, conversation?.members ?? [])
  if (conversation?.kind === 'direct') {
    return <span className="mt-1 text-[10px] text-muted-foreground">{confirmations.length ? '已读' : '已发送'}</span>
  }
  if (!confirmations.length) return <span className="mt-1 text-[10px] text-muted-foreground">已发送</span>

  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button type="button" variant="link" size="xs" className="mt-1 h-auto p-0 text-[10px] text-muted-foreground hover:text-primary">
          {confirmations.length} 人已读
        </Button>
      </PopoverTrigger>
      <PopoverContent align="end" className="w-72 p-2" aria-label="已读成员">
        <p className="px-2 pb-1.5 text-[11px] font-semibold text-muted-foreground">实际确认时间</p>
