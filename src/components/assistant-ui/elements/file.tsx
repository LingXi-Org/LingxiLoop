...
function FileRoot({ className, variant, size, children, ...props }: FileRootProps) {
  return <div data-slot="file-root" data-variant={variant} data-size={size} className={cn(fileVariants({ variant, size, className }))} {...props}>{children}</div>
}
...
function FileIconDisplay({ mimeType, className, children, ...props }: FileIconDisplayProps) {
  ...
  return <span data-slot="file-icon" className={cn("text-muted-foreground shrink-0", className)} {...props}>{children ?? <IconComponent className="size-5" />}</span>
}

function FileName({ className, children, ...props }: React.ComponentProps<"span">) {
  return <span data-slot="file-name" className={cn("min-w-0 flex-1 truncate font-medium", className)} {...props}>{children || "未命名文件"}</span>
}

type FileSizeProps = React.ComponentProps<"span"> & { bytes: number }

function FileSize({ bytes, className, ...props }: FileSizeProps) {
  return <span data-slot="file-size" className={cn("text-muted-foreground shrink-0", className)} {...props}>{formatFileSize(bytes)}</span>
}
...
function FileDownload({ data, mimeType, filename, sourceType, className, children, ...props }: FileDownloadProps) {
  ...
  return (
    <a
      ...
      className={cn("text-muted-foreground hover:bg-accent hover:text-accent-foreground shrink-0 rounded-md p-1 transition-colors", className)}
      {...props}
    >
