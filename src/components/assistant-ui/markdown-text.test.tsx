...
function Preview({ text, running, claims, inlineCitations, interrupted = false, animateEntry = false }: { text: string; running: boolean; claims?: MarkdownConfidenceClaim[]; inlineCitations?: boolean; interrupted?: boolean; animateEntry?: boolean }) {
  ...
  function CitedMessage() { return <MessagePrimitive.Parts components={{ Text: () => <MarkdownText segmented confidenceClaims={claims} inlineCitations={inlineCitations} interrupted={interrupted} animateEntry={animateEntry} /> }} /> }
...
test('streamed paragraphs only reveal complete blocks, including the final block on completion', () => {
  for (const [text, liveCount, finalCount] of [
    ...
    for (const inlineCitations of [true, false]) {
      for (const running of [true, false]) {
        ...
        const count = running ? liveCount : finalCount
        assert.equal((html.match(/class="im-markdown-bubble"/g) ?? []).length, count)
        assert.equal(html.includes('第一段'), count > 0)
      }
    }
  }
})
...
test('unfinished structural blocks stay buffered across blank lines and interruption', () => {
  for (const tail of ['- 一\n\n- 二', '> 引用\n>\n> 继续', '```js\nconst x = 1\n\n', '| 表头 |\n| --- |\n| 值 |']) {
    for (const interrupted of [false, true]) {
      ...
      assert.equal((html.match(/class="im-markdown-bubble"/g) ?? []).length, 1)
      assert.match(html, /已展示/)
      assert.doesNotMatch(html, /<ul|<blockquote|<table|<pre/)
    }
  }
})
...
test('new drafts show only body text and never activate citation navigation or stale confidence', () => {
  for (const running of [true, false]) {
    ...
    assert.equal(html.includes('第二段'), !running)
