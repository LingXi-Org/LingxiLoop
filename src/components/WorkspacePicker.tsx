    ...
    <main className="min-h-screen overflow-auto bg-app px-5 py-8 text-ink sm:px-10 lg:px-16">
      <div className="mx-auto max-w-[1180px]">
        <header className="flex flex-col gap-6 border-b border-hairline pb-8 sm:flex-row sm:items-end sm:justify-between">
          ...
          {companies.length > 1 && (
            <label className="text-xs font-semibold text-ink-secondary">组织
              <select value={activeCompanyId ?? ''} onChange={(event) => setActiveCompany(event.target.value)} className="mt-1 block h-10 min-w-52 rounded-xl border border-hairline bg-panel px-3 text-sm text-ink outline-none focus:border-accent">
        ...
        <div className="mb-4 flex items-center gap-5 text-xs font-semibold">
          <button onClick={() => setShowArchived(false)} className={!showArchived ? 'text-accent' : 'text-ink-secondary'}>活跃工作区</button>
            ...
            : visible.length === 0 ? <div className="grid min-h-64 place-items-center rounded-2xl border border-dashed border-hairline bg-panel/40 text-sm text-ink-secondary">没有符合条件的工作区</div>
              : <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {visible.map((workspace) => (
                  <button key={workspace.id} type="button" disabled={workspace.status === 'archived'} onClick={() => void select(workspace.id)} className="group min-h-52 rounded-2xl border border-hairline bg-panel p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-accent/40 hover:shadow-lg disabled:cursor-default disabled:opacity-70 disabled:hover:translate-y-0">
                    ...
                    <h2 className="mt-5 truncate text-lg font-semibold text-ink group-hover:text-accent">{workspace.name}</h2>
                    <p className="mt-1 line-clamp-2 min-h-10 text-xs leading-5 text-ink-secondary">{workspace.description || '空白工作区，等待你的第一份资料或第一次对话。'}</p>
