...
import type { ApiCompanyMember, ApiCompanyProfile, ApiCourse, ApiCourseInvitation, ApiCourseMember } from '@/api/contracts'
          ...
          <section className="rounded-2xl border border-hairline bg-panel p-5">
            ...
            {selectedCourse && <div className="space-y-5">
              ...
              {selectedCourse.canManage && <>
                ...
                <div className="space-y-1">{invitations.map((invitation) => <div key={invitation.id} className="flex items-center gap-2 py-1 text-[10px] text-ink-secondary"><span className="flex-1">{invitation.email ?? '公开链接'} · {invitation.role} · {invitation.useCount}/{invitation.maxUses}</span><span>{invitation.status}</span>{invitation.status === 'active' && <button onClick={async () => { await learningApi.revokeCourseInvitation(selectedCourse.id, invitation.id); await openCourse(selectedCourse) }}>撤销</button>}</div>)}</div>
              </>}
            </div>}
          </section>
        </div>}
