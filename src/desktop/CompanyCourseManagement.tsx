...
export function CompanyCourseManagement() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [courseRows, projectRows, company] = await Promise.all([
        api.listCourses(), api.listProjects(), api.getCompany(companyId),
      ])
      setCourses(courseRows); setProjects(projectRows); setProfile(company)
      if (isAdmin) setMembers(await api.listCompanyMembers(companyId))
  ...
  const openCourse = async (course: ApiCourse) => {
    ...
    try {
      const [memberRows, invitationRows] = await Promise.all([api.listCourseMembers(course.id), api.listCourseInvitations(course.id)])
  ...
  const createCourse = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const course = await api.createCourse({ name: String(data.get('name') ?? ''), description: String(data.get('description') ?? '') })
  ...
  const createInvite = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = new FormData(event.currentTarget)
    try {
      const invitation = await api.createCourseInvitation(selectedCourse.id, {
    ...
    <main className="h-full overflow-y-auto bg-app p-6 text-ink">
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        {tab === 'courses' && <div className="grid gap-5 lg:grid-cols-[1fr_1.15fr]">
          ...
          <section className="rounded-2xl border border-hairline bg-panel p-5">
            ...
            {selectedCourse && <div className="space-y-5">
              <div className="flex items-center justify-between"><div><h2 className="text-[18px] font-semibold">{selectedCourse.name}</h2><p className="text-[11px] text-ink-secondary">Project: {selectedCourse.projectId}</p></div>{selectedCourse.canManage && <button className={`${button} bg-raised`} onClick={async () => { await api.archiveCourse(selectedCourse.id, selectedCourse.status !== 'archived'); setSelectedCourse(null); await load() }}><IconArchive size={15}/></button>}</div>
              {selectedCourse.canManage && <>
                <div><h3 className="mb-2 text-[12px] font-semibold">课程成员</h3><div className="space-y-2">{courseMembers.map((member) => <div key={member.id} className="flex items-center gap-2 rounded-xl bg-raised p-2"><span className="min-w-0 flex-1"><b className="block truncate text-[12px]">{member.name}</b><small className="text-[10px] text-ink-secondary">{member.email}</small></span><select className="rounded-lg bg-panel p-1 text-[11px]" value={member.role} onChange={async (event) => { await api.updateCourseMember(selectedCourse.id, member.id, event.target.value as 'teacher' | 'learner'); await openCourse(selectedCourse) }}><option value="teacher">teacher</option><option value="learner">learner</option></select><button aria-label="移除" onClick={async () => { if (confirm(`从课程移除 ${member.name}？`)) { await api.removeCourseMember(selectedCourse.id, member.id); await openCourse(selectedCourse) } }}><IconTrash size={14}/></button></div>)}</div></div>
                ...
                <div className="space-y-1">{invitations.map((invitation) => <div key={invitation.id} className="flex items-center gap-2 py-1 text-[10px] text-ink-secondary"><span className="flex-1">{invitation.email ?? '公开链接'} · {invitation.role} · {invitation.useCount}/{invitation.maxUses}</span><span>{invitation.status}</span>{invitation.status === 'active' && <button onClick={async () => { await api.revokeCourseInvitation(selectedCourse.id, invitation.id); await openCourse(selectedCourse) }}>撤销</button>}</div>)}</div>
              </>}
            </div>}
          </section>
        </div>}
        ...
        {tab === 'organization' && isAdmin && profile && <div className="grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
          <form className="space-y-3 rounded-2xl border border-hairline bg-panel p-5" onSubmit={async (event) => { event.preventDefault(); const data = new FormData(event.currentTarget); await api.updateCompany(profile.id, { name: String(data.get('name')), description: String(data.get('description')) }); await load() }}><h2 className="text-[14px] font-semibold">组织资料</h2><input className={field} name="name" defaultValue={profile.name}/><input className={`${field} opacity-70`} value={profile.slug} readOnly/><textarea className={field} name="description" rows={4} defaultValue={profile.description}/><button className={`${button} w-full bg-accent text-white`}>保存</button></form>
          <section className="rounded-2xl border border-hairline bg-panel p-5"><div className="mb-3 flex items-center justify-between"><h2 className="text-[14px] font-semibold">组织成员</h2><button className={`${button} bg-accent text-white`} onClick={() => setCompanyInviteOpen(true)}>组织邀请</button></div><div className="space-y-2">{members.map((member) => <div key={member.id} className="flex items-center gap-2 rounded-xl bg-raised p-3"><span className="min-w-0 flex-1"><b className="block truncate text-[12px]">{member.name}</b><small className="text-[10px] text-ink-secondary">{member.email} · {member.courses.length} courses</small></span>{member.role === 'owner' ? <span className="text-[11px]">owner</span> : <><select className="rounded-lg bg-panel p-1 text-[11px]" value={member.role} onChange={async (event) => { await api.updateCompanyMember(profile.id, member.id, event.target.value as 'admin' | 'member'); await load() }}><option value="admin">admin</option><option value="member">member</option></select><button onClick={async () => { if (confirm(`从组织移除 ${member.name}？`)) { await api.removeCompanyMember(profile.id, member.id); await load() } }}><IconTrash size={14}/></button></>}</div>)}</div></section>
        </div>}
      </div>
