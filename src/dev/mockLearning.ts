...
const now = new Date()
const iso = (dayOffset: number, hour = 19) => {
...
let course: LearningCourse = {
  ...
  title: '线性代数：从概念到迁移项目',
  description: '用显式 Mission、分层练习、证据评价和间隔复习掌握线性代数。',
  status: 'active',
...
let activities: LearningActivity[] = [
  ...
  { id: 'activity-markov', courseId, title: '迁移项目：Markov 链长期状态', instructions: '选择一个新的转移矩阵，用谱分解解释长期状态并讨论假设。', type: 'project', status: 'draft', evaluationMode: 'teacher_required', targetLevel: 4, rubric: [{ criterion: '迁移情境' }, { criterion: '谱方法' }, { criterion: '限制反思' }], objectiveIds: ['obj-transfer'], dueAt: iso(14) },
...
let missions = [{
  ...
  steps: [
    ...
    { id: 'step-check', type: 'check', description: '独立完成换基 assessment', successCriteria: '无提示完成并用向量回代验证', status: 'open', position: 2 },
...
},{id:'mission-markov-project',courseId,learnerId:'mock-me',conversationId:'mock-lab-room',triggerClientMsgNo:'mock-lab-goal',goal:'完成 Markov 链迁移项目',successCriteria:'实验可复现并经独立验证',missionKind:'project',coordinatorAgentId:'mock-forge',coordinatorName:'Forge',status:'active',steps:[
  ...
  {id:'step-project-check',type:'check',description:'由独立验证者检查反例和适用条件',successCriteria:'Trace 提交 verdict 与反证检查',status:'completed',position:1,outcome:'Trace 支持局部结论并限定适用范围',completionReportId:'mock-report-trace'},
...
let evidence: Array<Record<string, unknown>> = [
  { id: 'attempt-3', activity_id: 'activity-change-basis', learner_id: 'mock-me', assistance: 'none', demonstrated_level: 3, feedback: '换基矩阵正确；回代验证完整。由于 assessment 需要教师确认，当前进入审核队列。', confidence: 0.91, status: 'pending', created_at: iso(-1, 20) },
...
let reviews: Array<Record<string, unknown>> = [
  { id: 'evaluation-change-basis', attempt_id: 'attempt-3', learner_id: 'mock-me', activity_title: '换基与矩阵表示检查', demonstrated_level: 3, confidence: 0.91, feedback: '独立完成且验证充分，等待教师确认 assessment。',source_report_id:'mock-report-forge',verifier_report_id:'mock-report-trace',builder_agent_id:'mock-forge',verifier_agent_id:'mock-trace',verifier_verdict:'supported',status: 'pending' },
  { id: 'evaluation-transfer', attempt_id: 'attempt-transfer', learner_id: 'mock-learner-2', activity_title: '迁移项目：Markov 链长期状态', demonstrated_level: 4, confidence: 0.86, feedback: '具备迁移证据，但 L4 必须由教师确认。',source_report_id:'mock-report-forge',verifier_report_id:null,builder_agent_id:'mock-forge',verifier_agent_id:null,verifier_verdict:null,status: 'pending' },
]
