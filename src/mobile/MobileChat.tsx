  ...
  return (
    <section className="chat-surface flex flex-col h-full overflow-x-hidden">
      ...
      >
        ...
        {uploading && (
          <div className="mb-2 text-[11.5px] text-ink-500 italic">正在上传...</div>
        )}
        {uploadError && (
        ...
        {mention && filteredMentions.length > 0 && (
          ...
          >
            <div className="px-3 pt-2 pb-1 text-[10px] font-bold uppercase tracking-[0.12em] text-ink-300">
              提及 {mention.query ? `· "${mention.query}"` : ''}
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-[13px] font-semibold text-ink-900 truncate">大家</div>
                      <div className="text-[11px] text-ink-500 truncate">通知该房间的所有成员</div>
                    </div>
                  </button>
                )
              }
    ...
    <section className="flex flex-col h-full bg-paper overflow-y-auto">
      ...
      >
        <div className="px-2 py-2.5 flex items-center gap-2">
          ...
          <h1 className="font-display font-medium text-[18px] text-ink-900" style={{ letterSpacing: '-0.01em' }}>详细信息</h1>
        </div>
      </header>
      ...
      {isGroup ? (
        ...
        >
          ...
          <div className="font-display italic text-[14px] text-ink-500">
            {memberPs.length} {memberPs.length === 1 ? "成员" : "成员"}
      ...
      {c.kind !== 'direct' && (
        <div className="py-4 px-5 border-b border-ink-100">
          {isGroup && (
            <div className="mb-4">
              <h4 className="text-[10.5px] font-bold text-ink-300 tracking-wider uppercase mb-2">负责人</h4>
              <select
                value={c.leaderId ?? ''}
                ...
                aria-label="更改群聊负责人"
              >
                {!c.leaderId && <option value="" disabled>选择一名活跃智能体</option>}
                {activeGroupAgents.map((agent) => <option key={agent.id} value={agent.id}>{agent.name}</option>)}
              </select>
              <p className="mt-1.5 text-[11px] text-ink-400">普通消息会唤醒该智能体；负责人可通过 @成员 ID 分派任务。</p>
            </div>
          )}
          <h4 className="text-[10.5px] font-bold text-ink-300 tracking-wider uppercase mb-3">成员</h4>
          <div className="grid grid-cols-3 gap-2 mb-3">
            <Stat n={String(memberPs.length)} l="成员" />
            <Stat n={String(agentCount)} l="智能体" />
            <Stat n={String(humanCount)} l="人工成员" />
          </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] text-ink-500 truncate font-display italic">
                      {p.kind === 'agent' ? (p.role ?? 'agent') : "人类队友"}
                    </div>
                  </div>
      ...
      {!isGroup && focus && (focus.tools ?? []).length > 0 && (
        <div className="py-4 px-5 border-b border-ink-100">
          <h4 className="text-[10.5px] font-bold text-ink-300 tracking-wider uppercase mb-3">工具</h4>
      ...
      {!isGroup && focus?.bio && (
        <div className="py-4 px-5 border-b border-ink-100">
          <h4 className="text-[10.5px] font-bold text-ink-300 tracking-wider uppercase mb-3">关于</h4>
          <div
      ...
      <div className="py-4 px-5">
        ...
        >
          离开对话
          <span className="block font-display italic text-[11px] text-ink-500 mt-0.5">特工在没有你的情况下继续</span>
        </button>
      </div>
    </section>
  )
