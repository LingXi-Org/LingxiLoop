...
test('attachment upload and loading surfaces share the preset shadcn primitives', () => {
  ...
  assert.match(message, /aria-label=\{`打开附件：\$\{attachment\.filename\}`\}/)
  assert.doesNotMatch(message, /@\/components\/ui\/attachment/)
  assert.match(message, /flex-row gap-2 overflow-x-auto/)
  assert.match(message, /className=\{conversationCardSize\.tile\}/)
