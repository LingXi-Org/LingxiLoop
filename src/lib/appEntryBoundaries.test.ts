...
test('shared entry keeps notification, analytics, and app surfaces behind dynamic boundaries', () => {
  ...
  const suspended = readFileSync(new URL('../features/admin/components/SuspendedScreen.tsx', import.meta.url), 'utf8')
  const waitlist = readFileSync(new URL('../features/admin/components/WaitlistConfirmedScreen.tsx', import.meta.url), 'utf8')
  ...
  assert.match(app, /lazy\(\(\) => import\('@\/features\/admin\/components\/AdminApp'\)/)
