...
test('desktop uses the shadcn v4 resizable source for persisted two and three panel layouts', async () => {
  const shell = await readFile(new URL('../desktop/DesktopApp.tsx', import.meta.url), 'utf8')
  const resizable = await readFile(new URL('../components/ui/resizable.tsx', import.meta.url), 'utf8')
  const css = await readFile(new URL('../styles/globals.css', import.meta.url), 'utf8')
  assert.match(shell, /DESKTOP_TWO_PANEL_LAYOUT_KEY/)
  assert.match(shell, /DESKTOP_THREE_PANEL_LAYOUT_KEY/)
  assert.match(shell, /<ResizablePanelGroup/)
  assert.match(shell, /orientation="horizontal"/)
  assert.match(shell, /onLayoutChanged=/)
  assert.match(shell, /id="conversations"[\s\S]*?id="conversation"[\s\S]*?id="detail"/)
...
test('desktop group context participates in the native three panel lifecycle', async () => {
  const shell = await readFile(new URL('../desktop/DesktopApp.tsx', import.meta.url), 'utf8')
  ...
  assert.match(shell, /detailPanelOpen = !canvasId/)
  assert.match(shell, /<ChatPane onOpenGroupContext=/)
  assert.match(chat, /aria-label="打开群聊上下文"/)
  assert.match(shell, /aria-label="调整上下文栏宽度"/)
  assert.match(shell, /aria-label="关闭上下文栏"/)
  assert.match(shell, /minSize=\{GROUP_PANEL_MIN\}/)
  assert.match(shell, /maxSize=\{GROUP_PANEL_MAX\}/)
...
test('Canvas bubble and full view share the Card surface and preview theme', async () => {
  ...
  assert.doesNotMatch(css, /--bubble-user: #1084FE/i)
